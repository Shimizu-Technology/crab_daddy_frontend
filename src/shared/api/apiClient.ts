...
axiosInstance.interceptors.request.use((config: InternalAxiosRequestConfig) => {
  ...
  config.headers.set('X-Frontend-ID', 'crab_daddy');
  config.headers.set('X-Frontend-Restaurant-ID', import.meta.env.VITE_RESTAURANT_ID || '4');
  ...
  if (token) {
    // For Crab Daddy frontend, always use restaurant ID 4
    ...
    config.headers.set('X-Restaurant-ID', crabDaddyRestaurantId);
    ...
    if (config.url && (needsRestaurantContext(config.url) || token)) {
      // Always use the Crab Daddy restaurant ID (4) for this frontend

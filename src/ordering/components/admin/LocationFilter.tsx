  ...
  return (
    <div className={`relative ${className}`}>
      <label htmlFor="location-filter" className="text-sm font-medium text-gray-700 mb-1 flex items-center">
        ...
        Filter by Location
        {selectedLocationId && (
          <span className="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#E87230] bg-opacity-20 text-[#E87230]">
            Active Filter
          </span>
        )}
      </label>
      <div className="relative">
        ...
        {isLoading && (
          <div className="absolute right-10 top-1/2 transform -translate-y-1/2 pointer-events-none">
            <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-[#E87230]"></div>
          </div>
        )}
      </div>
      {selectedLocation && (
